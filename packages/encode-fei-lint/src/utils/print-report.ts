...
export default (results: ScanResult[], fix: boolean) => {
    let output = '\n'
    let errorCount = 0
    let warningCount = 0
    ...
    const transformMessage = ({ line, column, message, rule, url, errored }) => {
    ...
    for (const result of results) {
        ...
        output += `${table(messages.map(transformMessage), { 
            align: ['l', 'r', 'l', 'l', 'l'] ,
            stringLength: (str) => stripansi(str).length //自定义如何计算字符串的显示宽度
        })}\n\n}`
    ...
    //修复日志
    if(fix){
        output += chalk.green('代码规范问题自动修复完成, 请通过git Diff 查看修复结果\n')
    }
    if(fix && total > 0){
        output += chalk.green('ps. 以上显示的是无法修复的问题, 需要手动进行修复\n')
    }
    ...
    console.log(chalk.reset(output)) //输出正常颜色
}
